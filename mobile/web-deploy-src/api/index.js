...
// %%RUKZ_...%% definidos em mobile/web/index.html são preenchidos abaixo.
const template = require('./_template.js');

const DASHBOARD_ORIGIN = 'https://rukz.vercel.app';
...
module.exports = async (req, res) => {
  ...
  // rukz — mesma casca, barbearia certa.
  ...
  const html = template
    .split("%%RUKZ_THEME_COLOR%%").join(escapeHtml(themeColor))
    .split("%%RUKZ_NAME%%").join(escapeHtml(name))
    .split("%%RUKZ_ICON_URL%%").join(escapeHtml(iconUrl))
    .split("%%RUKZ_MANIFEST_URL%%").join(escapeHtml(manifestUrl));
