...
function passwordStrength(password: string): { score: number; label: string; color: string } {
  if (!password) return { score: -1, label: "", color: "bg-traco-forte" };
  let score = 0;
...
function RegisterForm() {
  ...
  const planOptions = [
    ...
    { value: "pro", label: "Pro", price: "R$ 350/mês", description: "Copiloto com IA, financeiro e assinatura" },
  ...
  return (
    <div className="w-full max-w-md">
      <h1 className="tipo-titulo text-4xl text-neve mb-3">
        Criar a sua conta
      </h1>
      <p className="text-cinza text-sm mb-1">
        Já tem conta?{" "}
        <Link href="/login" className="text-ouro hover:text-ouro-claro font-medium transition-colors">
          Entrar
        </Link>
      </p>
      ...
      <div className="mb-7">
        <div className="flex gap-1.5">
          {[1, 2, 3].map((n) => (
            <span key={n} className={`h-1 flex-1 rounded-full transition-colors ${step >= n ? "bg-ouro" : "bg-traco-forte"}`} />
          ))}
        </div>
        <p className="mt-2 text-xs text-cinza">
          Etapa {step} de 3 · <span className="text-neve/80">{PASSOS[step - 1]}</span>
        </p>
      </div>
      ...
      {step > 1 && planoEscolhido && (
        <div className="mb-5 flex items-center justify-between rounded-xl border border-traco-forte bg-grafite/70 px-3.5 py-2.5">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-neve">{planoEscolhido.label}</p>
            <p className="text-xs text-cinza">{planoEscolhido.price}</p>
          </div>
          <button type="button" onClick={() => setStep(1)} className="shrink-0 text-xs font-medium text-ouro transition-colors hover:text-ouro-claro">
            trocar
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {step === 1 ? (
          ...
          <div className="space-y-2">
            {planOptions.map((p) => {
              ...
              return (
                <label key={p.value} className="block cursor-pointer">
                  ...
                  <div className={`rounded-xl border p-4 transition-colors ${escolhido ? "border-ouro bg-ouro/[0.07]" : "border-traco-forte bg-grafite/70 hover:border-traco-forte"}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-neve">{p.label}</span>
                          {p.value === "pro" && (
                            <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-semibold text-neve">mais escolhido</span>
                          )}
                        </div>
                        <p className="mt-1 text-xs leading-relaxed text-cinza">{p.description}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        <p className="text-sm font-semibold text-neve">{p.price}</p>
                        {escolhido && <Check className="ml-auto mt-1 h-4 w-4 text-ouro" />}
                      </div>
                    </div>
                  </div>
                </label>
            ...
            <p className="pt-1 text-xs leading-relaxed text-cinza/60">
          ...
          <>
            <div>
              <label className="block text-[13px] font-medium text-neve/80 mb-1.5">
                Seu nome completo
              </label>
              <input type="text" autoComplete="name" placeholder="João Silva" {...register("name")}
                className="w-full h-11 px-3.5 bg-grafite/70 border border-traco-forte rounded-xl text-neve placeholder:text-cinza/60 focus:outline-none focus:border-ouro transition-colors text-sm" />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="Crie uma senha forte" {...register("password")}
                  className="w-full h-11 px-3.5 pr-11 bg-grafite/70 border border-traco-forte rounded-xl text-neve placeholder:text-cinza/60 focus:outline-none focus:border-ouro transition-colors text-sm" />
              ...
              {password && (
                <>
                  <div className="mt-2 flex items-center gap-1.5">
                    {[0, 1, 2, 3].map((i) => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= strength.score ? strength.color : "bg-traco-forte"}`} />
                    ))}
                    <span className="text-[10px] text-cinza ml-1 whitespace-nowrap">{strength.label}</span>
                  </div>
          ...
          <>
            ...
            <div>
              ...
              <div className="flex items-center">
                <span className="h-11 px-3 bg-grafite border border-traco-forte border-r-0 rounded-l-xl text-cinza text-xs flex items-center whitespace-nowrap">
                  {baseDeAgendamento()}
                </span>
                <input
                  type="text"
                  ...
                  className="flex-1 min-w-0 h-11 px-3.5 bg-grafite/70 border border-traco-forte rounded-r-xl text-neve placeholder:text-cinza/60 focus:outline-none focus:border-ouro transition-colors text-sm"
                />
              </div>
              {errors.barbershopSlug ? (
                <p className="text-xs text-red-400 mt-1.5">{errors.barbershopSlug.message}</p>
              ) : slugStatus === "checking" ? (
                <p className="text-xs text-cinza mt-1.5 flex items-center gap-1">
                ...
                barbershopSlug && (
                  <p className="text-xs text-cinza/60 mt-1.5">Use apenas letras minúsculas, números e hífen (mín. 3 caracteres)</p>
                )
              )}
            </div>
            <div className="grid grid-cols-[1fr_auto] gap-3">
              ...
              <div>
                ...
                <select
                  {...register("state")}
                  className="h-12 w-20 px-3 bg-grafite border border-traco-forte rounded-2xl text-neve focus:outline-none focus:ring-2 focus:ring-ouro/50 focus:border-ouro/60 transition-all text-sm appearance-none text-center"
                >
              ...
              <div>
                <label className="block text-[13px] font-medium text-neve/80 mb-1.5">
                  CNPJ <span className="text-ouro-claro/80 normal-case font-normal">(obrigatório)</span>
                </label>
                <input
                  type="text"
                  inputMode="numeric"
        ...
        <button
          ...
          className="w-full h-12 bg-ouro text-preto font-semibold rounded-xl hover:bg-ouro-claro transition-colors disabled:opacity-50 disabled:hover:bg-ouro flex items-center justify-center gap-2 text-sm"
        >
          {isLoading ? (
        ...
        {step > 1 && (
          <button
            ...
            className="w-full text-sm text-cinza hover:text-neve/80 transition-colors"
          >
            ← Voltar
          </button>
        )}
      ...
      <p className="mt-6 text-xs text-cinza/60 text-center">
        Ao criar uma conta, você concorda com nossos{" "}
        <Link href="/termos" className="text-cinza hover:underline">
          Termos de Uso
        </Link>{" "}
        e{" "}
        <Link href="/privacidade" className="text-cinza hover:underline">
