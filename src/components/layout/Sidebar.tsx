...
const navItems = [
  ...
  { label: "Servicos", href: "/dashboard/services", icon: Scissors },
    ...
    <aside style={{ width: w, minWidth: w }} className="fixed left-0 top-0 h-screen flex flex-col transition-all duration-300 z-40 bg-zinc-950 border-r border-zinc-800/60">
      ...
      {!collapsed && (
        <div className="mx-3 mb-3 p-3 bg-gradient-to-br from-amber-500/10 to-yellow-500/5 border border-amber-500/20 rounded-xl">
          ...
          <p className="text-xs text-zinc-500">Multiplas unidades e API.</p>
          <button className="mt-2 w-full text-xs font-semibold text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 py-1.5 rounded-lg transition-colors">Fazer upgrade</button>
        </div>
      )}
      ...
      <div className="px-2 pb-3 border-t border-zinc-800/60 pt-3 space-y-0.5">
        ...
        <Link href="/login" className={cn("flex items-center rounded-lg transition-all text-zinc-600 hover:bg-red-500/10 hover:text-red-400", collapsed ? "justify-center h-10" : "gap-3 px-3 py-2.5")}>
